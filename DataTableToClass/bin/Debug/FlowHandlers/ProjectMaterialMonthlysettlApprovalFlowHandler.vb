...
Public Class ProjectMaterialMonthlysettlApprovalFlowHandler
...
#Region "FlowStep相关"
    ...
    Public Function GetFlowStepByID(id As Integer) As ApprovalBase Implements IFlowHandler.GetFlowStepByID
        Return EntityHelper.GetFristDefault(Of ProjectMaterialMonthlysettlApprovalModel)(Function(x) x.ID = id)
    End Function
    ...
    Public Function GetNextFlowStep(lid As Integer) As ApprovalBase Implements IFlowHandler.GetNextFlowStep
        Return FlowHandlerCommon.GetNextStepApproval(Of ProjectMaterialMonthlysettlApprovalModel)(lid)
    End Function
    ...
    Public Function GetFirstFlowStep(lid As Integer) As ApprovalBase Implements IFlowHandler.GetFirstFlowStep
        Return FlowHandlerCommon.GetFirstFlowStep(Of ProjectMaterialMonthlysettlApprovalModel)(lid)
    End Function
    ''' <summary>
    ''' 获取当前审批
    ''' </summary>
    ...
    Public Function GetCurrentFlowStep(lid As Integer) As ApprovalBase Implements IFlowHandler.GetCurrentFlowStep
        Return FlowHandlerCommon.GetCurrentStepApproval(Of ProjectMaterialMonthlysettlApprovalModel)(lid)
    End Function

    Public Function GetFlowListInfo(projectId As Integer, lid As Integer) As String Implements IFlowHandler.GetFlowListInfo
        Dim listModel = EntityHelper.GetSelect(Of ProjectMaterialMonthlysettlApprovalModel)(Function(x) x.PID = projectId AndAlso x.SID = lid)
    ...
    Public Function GetFlowlist(lid As Integer) As System.Collections.Generic.List(Of ApprovalBase) Implements IFlowHandler.GetFlowlist
        Return FlowHandlerCommon.GetFlowlist(Of ProjectMaterialMonthlysettlApprovalModel)(lid).ToList
    End Function
    ''' <summary>
    ''' 获取下一步审批
    ''' </summary>
    ...
    Public Function GetNextStepByID(lid As Integer, stepId As Integer) As ApprovalBase Implements IFlowHandler.GetNextStepByID
        Dim listEntity = FlowHandlerCommon.GetFlowlist(Of ProjectMaterialMonthlysettlApprovalModel)(lid)
    ...
    Public Function Update(model As ApprovalBase) As Boolean Implements IFlowHandler.Update
        Return EntityHelper.Update(Of ProjectMaterialMonthlysettlApprovalModel)(model)
    End Function
...
#Region "审批处理相关"
    ...
    Public Function SetFlowStepUser(projectId As Integer, stepId As Integer, approver As String) As Boolean Implements IFlowHandler.SetFlowStepUser
        Return FlowHandlerCommon.SetFlowStepApprover(Of ProjectMaterialMonthlysettlApprovalModel)(projectId, stepId, approver)
    End Function
    ...
    Public Function IsNeedSetFlowUser(applyType As ApprovalTypeEnum,
        ...
        Dim obj = ProjectMaterialMonthlysettlApprovalLidService.Get(lid)
        Dim curFlowStep = ProjectMaterialMonthlysettlApprovalService.GetCurrentStepApproval(lid)

        Return FlowHandlerCommon.IsNeedSetFlowUser(applyType, projectId, lid, obj.PlanCode, obj.PlanName, curUserId, approvalResult, curFlowStep, Me)
    End Function
    ...
    Public Function IsLastStepFlowStep(lid As Integer, stepId As Integer) As Boolean Implements IFlowHandler.IsLastStepFlowStep
        Return FlowHandlerCommon.IsLastStepFlowStep(Of ProjectMaterialMonthlysettlApprovalModel)(lid, stepId)
    End Function
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Function IsExistApprovalFlow(lid As Integer) As Boolean Implements IFlowHandler.IsExistApprovalFlow
        Return FlowHandlerCommon.IsExistApprovalFlow(Of ProjectMaterialMonthlysettlApprovalModel)(lid)
    End Function
    ...
    Public Sub DoFuncAfterFinishFlow(aplType As Integer, projectId As Integer, lastApprovalStepModel As ApprovalBase) Implements IFlowHandler.DoFuncAfterFinishFlow
        Call ProjectMaterialMonthlysettlApprovalService.FinishFunc(aplType, projectId, lastApprovalStepModel)
    End Sub
    ...
    Public Function SubmitFlow(applyType As ApprovalTypeEnum, curUserId As Integer, projectId As Integer, lid As Integer, stepId As Integer, opinion As String, approvalResult As Integer) As Boolean Implements IFlowHandler.SubmitFlow
        Dim lidCode As String = ProjectMaterialMonthlysettlApprovalLidService.GetCode(lid)
        ...
        Return FlowHandlerCommon.SubmitApproval(applyType, curUserId, projectId, lid, lidCode, opinion, approvalResult,
                                               AddressOf ProjectMaterialMonthlysettlApprovalService.FinishFunc,
                                               AddressOf ProjectMaterialMonthlysettlApprovalService.RejectFunc,
                                               curFlowStep, Me)
    End Function
    ...
    Public Function GetOriginator(lid As Integer) As String Implements IFlowHandler.GetOriginator
        Return FlowHandlerCommon.GetOriginator(Of ProjectMaterialMonthlysettlApprovalModel)(lid)
    End Function
...
#Region "单据相关"
    ...
    Public Function GetLidInfo(id As Integer) As LidInfo Implements IFlowHandler.GetLidInfo
        Dim model = ProjectMaterialMonthlysettlApprovalLidService.Get(Function(x) x.ID = id)
    ...
    Public Function GetLidInfo(lidCode As String) As LidInfo Implements IFlowHandler.GetLidInfo
        Dim model = ProjectMaterialMonthlysettlApprovalLidService.Get(Function(x) x.PlanCode = lidCode)
        Dim info = New LidInfo
        With info
            .ID = model.ID
            .Name = model.PlanName
...
#Region "传阅相关"
    ...
    Public Function SetCirculatedUser(projectId As Integer, stepId As Integer, CirculatedUserIds As String) As Boolean Implements IFlowHandler.SetCirculatedUser
        Return FlowHandlerCommon.SetCirculatedUser(Of ProjectMaterialMonthlysettlApprovalModel)(projectId, stepId, CirculatedUserIds)
    End Function
    ...
    Public Sub UpdateIsView(userId As Integer, lid As Integer) Implements IFlowHandler.UpdateIsView
        Call FlowHandlerCommon.UpdateIsView(Of ProjectMaterialMonthlysettlApprovalModel)(userId, lid)
    End Sub
...
#Region "FlowRecord相关"
    ...
    Public Sub UpdateFlowRecord(applyType As ApprovalTypeEnum, lid As Integer) Implements IFlowHandler.UpdateFlowRecord
        Call ProjectMaterialMonthlysettlApprovalLidService.UpdateFlowRecord(applyType, lid)
    End Sub
...
#Region "流程督办"
    ...
    Public Function ChangeCurStep(aplType As Integer, projectId As Integer, lid As Integer, lidCode As String, userId As Integer, flowStep As ApprovalBase, approvalHandler As IFlowHandler) As Boolean Implements IFlowHandler.ChangeCurStep
        Return FlowHandlerCommon.ChangeCurStep(Of ProjectMaterialMonthlysettlApprovalModel)(aplType, projectId, lid, lidCode, userId, flowStep, Me)
    End Function
    ''' <summary>
    ''' 完成审批
    ''' </summary>
    ...
    Public Function EndApproval(applyType As Integer, projectId As Integer, lid As Integer, lidCode As String) As Boolean Implements IFlowHandler.EndApproval
        Return FlowHandlerCommon.EndApproval(Of ProjectMaterialMonthlysettlApprovalModel)(applyType, projectId, lid, lidCode, Me)
    End Function
    ...
    Public Function RemindApprover(aplType As Integer, projectId As Integer, lid As Integer, lidCode As String, flowStep As ApprovalBase) As Boolean Implements IFlowHandler.RemindApprover
        Return FlowHandlerCommon.RemindApprover(Of ProjectMaterialMonthlysettlApprovalModel)(aplType, projectId, lid, lidCode, flowStep, Me)
    End Function
#End Region
...
#Region "企业微信相关"
    ...
    Public Sub BuildWebChatMSG(applType As Integer, projectId As Integer, lid As Integer, stepName As String, msgType As Integer, ByRef title As String, ByRef description As String, ByRef btnText As String) Implements IFlowHandler.BuildWebChatMSG
        ProjectMaterialMonthlysettlApprovalLidService.BuildWebChatMSG(applType, projectId, lid, stepName, msgType, title, description, btnText)
    End Sub
    ...
    Public Function WebChatBuildListInfo(dataType As Integer, apprlType As Integer, lid As Integer) As String Implements IFlowHandler.WebChatBuildListInfo
        Return ProjectMaterialMonthlysettlApprovalLidService.WebChatBuildListInfo(dataType, apprlType, lid)
    End Function
#End Region
